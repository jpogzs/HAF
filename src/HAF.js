...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['m-chkLive','m-chkTraining','q-chkLive','q-chkTraining'].forEach(id => {
    const btn = document.getElementById(id);
    if (btn && localStorage.getItem(btn.dataset.key) === 'true') btn.classList.add(btn.dataset.active);
  });
...
<td>${mText}${pmText}</td>`;
            ...
            initColumnVisibility('measure-table','hiddenCols_measure','m-columnControls');
            m_pendingFetches--;
            if(m_pendingFetches===0){sortTableByColumn('measure-table',10,true);colorDuplicateMeasureUsernames();startDynamicTimers('m-data');if(!reloadTimerStarted)startReloadTimer();}
          })
          .catch(err=>{
...
function colorDuplicateMeasureUsernames(){
  ...
  rows.forEach(row=>{
    ...
    if(cells.length>=5&&cells[2].textContent.trim().includes('Being Measured')&&row.dataset.type!=='training'){
      const u=cells[4].textContent.trim(),t=cells[5].textContent.trim();
  ...
  rows.forEach(row=>{
    ...
    if(cells.length>=5&&cells[2].textContent.trim().includes('Being Measured')&&row.dataset.type!=='training'){
      const u=cells[4].textContent.trim(),t=cells[5].textContent.trim();
      if(u&&uC.get(u)>1)cells[4].style.color='red';
      if(t&&tC.get(t)>1)cells[5].style.color='red';
    }
  });
}
