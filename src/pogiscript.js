...
function colorDuplicateUsernames() {
  ...
  // First pass: count occurrences only in "Being Measured" rows
  rows.forEach(row => {
    const cells = row.cells;
    if (cells.length >= 4 && cells[1].textContent.trim().includes("Being Measured")) {
      const userName = cells[2].textContent.trim();
      const techUsername = cells[3].textContent.trim();
  ...
  // Second pass: color duplicates in "Being Measured" rows
  rows.forEach(row => {
    const cells = row.cells;
    if (cells.length >= 4 && cells[1].textContent.trim().includes("Being Measured")) {
      const userNameCell = cells[2];
      const techUsernameCell = cells[3];
              ...
              return fetch(`https://api.cmh.platform-prod2.evinternal.net/operations-center/api/TaskState/id/${item.taskStateID}`)
                ...
                .then(task => {
                  ...
                  return fetch(`https://api.cmh.platform-prod2.evinternal.net/operations-center/api/TaskState/${task.taskID}`)
                    .then(r => r.ok ? r.json() : Promise.reject())
                    .then(task2 => {
                      // count rework instances in ANY field
                      let reworkCount = (JSON.stringify(task2).match(/Rework/g) || []).length;
                      return { report, productName, pmText, task, reworkCount };
                    });
                });
            })
            .then(({ report, productName, pmText, task, reworkCount }) => {
              ...
              return fetch(`https://api.cmh.platform-prod2.evinternal.net/operations-center/api/User/id?ids=${userID}`)
                .then(r => r.ok ? r.json() : Promise.reject())
                .then(user => ({ report, productName, pmText, task, reworkCount, user }));
            })
            .then(({ report, productName, pmText, task, reworkCount, user }) => {
              ...
              return fetch(`https://api.cmh.platform-prod2.evinternal.net/operations-center/api/Report/${item.reportID}/measurement-items`)
                .then(r => r.ok ? r.json() : Promise.reject())
                .then(mItems => ({ report, productName, pmText, task, reworkCount, user, mItems }));
            })
            .then(({ report, productName, pmText, task, reworkCount, user, mItems }) => {
...
<td>
  ${isRTM
    ? `Ready To Measure${reworkCount > 0 ? ` <span style="color: ${reworkCount > 1 ? 'red' : 'inherit'};">(Rework ${reworkCount}x)</span>` : ""}`
    : `Being Measured${reworkCount > 0 ? ` <span style="color: ${reworkCount > 1 ? 'red' : 'inherit'};">(Rework ${reworkCount}x)</span>` : ""}`}
</td>
